...
async function syncUserLikes() {
    try {
        ...
        if (response.ok) {
            ...
            window.userLikes = new Set(likes.map(l => l.id));
            console.log('[Dashboard] Synced likes:', window.userLikes.size);
        }
    } catch (error) {
...
async function loadTrendingTracks() {
    ...
    try {
        ...
        // For simplicity, we'll bypass cache if filters are active during this task
        if (trendingCache && !genre && time === 'allTime') {
            renderTracks(trendingCache, 'For You');
            return;
        }

        let tracks = [];
        let title = 'Trending Tracks';

        // If specific filters are selected, use the music API directly
        if (genre || time !== 'allTime') {
            const response = await fetch(`${API_URL}/api/music/songs?time=${time}&genre=${genre}`);
            if (response.ok) {
                const data = await response.json();
                tracks = data.songs || [];
                title = `Trending ${genre || ''} (${time === 'allTime' ? 'All Time' : 'This ' + time.charAt(0).toUpperCase() + time.slice(1)})`;
            }
        } else {
            // Default "For You" view from recommendations
            let response = await fetch(`${API_URL}/api/recommendations/dashboard`);
            let data;
            if (response.ok) {
            ...
            // Fallback to general trending if no recommendations
            if (tracks.length === 0) {
                response = await fetch(`${API_URL}/api/recommendations/trending`);
                if (response.ok) {
                    data = await response.json();
        ...
        document.getElementById('feedTitle').textContent = 'For You';
...
function createArtistCardHTML(artist) {
    return `
        <a href="/artist/${artist.id || artist._id}" style="text-decoration: none; color: inherit;">
            <div style="background: var(--bg-card); border-radius: 12px; padding: 16px; text-align: center; transition: transform 0.2s; cursor: pointer;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform='translateY(0)'">
                <div style="width: 100%; aspect-ratio: 1; background: var(--bg-secondary); border-radius: 50%; overflow: hidden; margin-bottom: 12px;">
                    ${artist.profilePicture ? `<img src="${artist.profilePicture}" alt="${artist.name}" style="width: 100%; height: 100%; object-fit: cover;">` : `<div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: 3rem; font-weight: 800; color: var(--text-secondary);">${artist.name.charAt(0)}</div>`}
                </div>
                <h4 style="margin: 0 0 4px 0; font-size: 1rem;">${artist.name}</h4>
    ...
    try {
        ...
        container.innerHTML = comments.map(comment => `
            <div style="display: flex; gap: 12px; padding: 16px 0; border-bottom: 1px solid var(--border-color);">
                <div style="width: 40px; height: 40px; border-radius: 50%; background: var(--accent-gradient); flex-shrink: 0;"></div>
                <div style="flex: 1;">
                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                        <span style="font-weight: 600; font-size: 0.9rem;">${comment.userName || 'Anonymous'}</span>
                        <span style="color: var(--text-muted); font-size: 0.8rem;">${formatTimeAgo(comment.createdAt)}</span>
                    </div>
                    <p style="color: var(--text-secondary); line-height: 1.5;">${comment.text}</p>
                </div>
            </div>
        `).join('');
    } catch (error) {
