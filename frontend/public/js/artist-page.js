        ...
        html += `
            <div style="background: var(--bg-card); border-radius: 12px; padding: 16px; transition: transform 0.2s; cursor: pointer;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform='translateY(0)'">
                <div style="width: 100%; aspect-ratio: 1; background: var(--bg-secondary); border-radius: 8px; overflow: hidden; margin-bottom: 12px;">
                    ${album.coverArtUrl ? `<img src="${album.coverArtUrl}" alt="${album.title}" style="width: 100%; height: 100%; object-fit: cover;">` : ''}
                </div>
                <h4 style="margin: 0 0 8px 0; font-size: 1rem;">${album.title}</h4>
...
window.toggleLike = async function (songId) {
    ...
    try {
        ...
        if (response.ok) {
            // Update UI manually for a smoother experience
            const indicators = document.querySelectorAll(`.audius-track-row[data-song-id="${songId}"] .like-indicator`);
            indicators.forEach(indicator => {
                const icon = indicator.querySelector('i');
                const isLiked = indicator.classList.contains('liked');
                if (!isLiked) {
                ...
                list.innerHTML = comments.map(c => `
                    <div class="comment-item">
                        ...
                        <div class="comment-body">
                            <div class="comment-user">User ${c.userId.substring(0, 8)}</div>
                            <div class="comment-text">${c.content}</div>
                            <div class="comment-date">${new Date(c.createdAt).toLocaleDateString()}</div>
                        </div>
                    </div>
                `).join('');
